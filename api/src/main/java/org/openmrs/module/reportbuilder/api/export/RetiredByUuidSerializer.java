...
/**
 * Serializer for retiredBy field that outputs just the UUID instead of the full User object. Unlike
 * creator, retiredBy is usually null (only set when entity is retired), so we handle null properly
 * instead of defaulting to system user.
 */
public class RetiredByUuidSerializer extends JsonSerializer<Object> {
	
	@Override
	public void serialize(Object value, JsonGenerator gen, SerializerProvider provider) throws IOException {
		if (value == null) {
			// For retiredBy, null means the entity is not retired, so serialize as null
			gen.writeNull();
			return;
		}
